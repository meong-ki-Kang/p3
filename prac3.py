...
for i in range(len(sk.index)) :
    for j in range(len(sk.iloc[i])) :
        sk.iloc[i, j]=float(sk.iloc[i, j].replace(',', ''))

for i in range(len(db.index)) :
    for j in range(len(db.iloc[i])) :
        db.iloc[i, j]=float(db.iloc[i, j].replace(',', ''))
        
for i in range(len(kospi.index)) :
    for j in range(len(kospi.iloc[i])) :
        kospi.iloc[i, j]=float(kospi.iloc[i, j].replace(',', ''))
...
kospi_layer1=LSTM(512, activation='relu')(kospi_input1)
kospi_layer1=Dropout(0.2)(kospi_layer1)
kospi_layer2=Dense(4096, activation='relu')(kospi_layer1)
kospi_layer3=Dropout(0.2)(kospi_layer2)
kospi_layer3=Dense(2048, activation='relu')(kospi_layer3)
kospi_layer3=Dropout(0.2)(kospi_layer3)
kospi_layer3=Dense(512, activation='relu')(kospi_layer3)
kospi_layer3=Dense(256, activation='relu')(kospi_layer3)
kospi_layer3=Dense(128, activation='relu')(kospi_layer3)
kospi_layer3=Dense(64, activation='relu')(kospi_layer3)
...
hist=model.fit([samsung_x_train, sk_x_train, db_x_train, kospi_x_train], samsung_y_train, epochs=2, batch_size=100, validation_split=0.2, callbacks=[es, cp])
